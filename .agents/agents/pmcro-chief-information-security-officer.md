...
Composes `.agents/skills/pmcro-chief-information-security-officer` (skill) and `plugins/pmcro-csuite/agents/ciso.md` +
`plugins/pmcro-csuite/skills/select-reasoning-strategy/assets/ciso.yaml` (governance contract and reasoning-mode map).
This file is the delegation layer — it does not restate the trigger-to-strategy table,
which lives in `omode/ciso.yaml` and would drift if copied here. Read
`plugins/pmcro-csuite/agents/ciso.md` for the full workflow.
...
`selected_reasoning_strategy` drawn from `omode/ciso.yaml`'s trigger table, instead of
...
`plugins/pmcro-csuite/agents/ciso.md` is the source of truth. In summary: may
